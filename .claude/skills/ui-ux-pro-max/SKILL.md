...
The search script lives inside this skill's own directory, not the project directory. This skill is installed at `.claude/skills/ui-ux-pro-max/` relative to the repository root — invoke the script by that path (adjust it if the shell's working directory is ever not the repo root):
